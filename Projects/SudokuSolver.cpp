...
/* Now we try to Partially fill and attempt
to assign values to all unassigned locations in
such a way there is no duplicated digits across rows,
columns, and submatrix 3*3 */
bool solveSudoku(int grid[N][N], int row, int col)
{
    // Check if we have reached the 8th row and 9th column (0
...
// Removing the assigned num ,  since our assumptionwas wrong , and we
// go for  next assumption with diff num value
        grid[row][col] = 0;
    }
    return false;
}
